...
class Playlist {
    ...
    public static function getAbbonniertePlaylists($userid)
    {
        $ret = json_decode(RestAPI::get("/playlists"));
        $idarray = array();
        $result = array();
        foreach ($ret->items as $items) {
            $id = $items->id;
            if (Playlist::isfollower($id)) {
                array_push($idarray, $id);
            }
        }
        foreach ($idarray as $playlistid) {
            array_push($result, json_decode(Playlist::getPlaylistById($playlistid)));
