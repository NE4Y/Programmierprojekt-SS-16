    ...
    for(var i=0; i<trackCount; i++){
       tracks[i].onclick = function(){
          ...
          trackSelect(this);
           ...
           var id = document.getElementsByClassName("currentTrack")[0].getAttribute("id");
           $.ajax({
               type: "POST",
               url: "./update",
               data: {id: id},
               success: function (result) {
                   ...
                   $('#video_title')[0].innerHTML = video.title;
                   $('#title')[0].innerHTML = video.title;
                   $('#user')[0].innerHTML = video.user;
                   $('#rating')[0].innerHTML = video.rating;
                   $('#comment_sec')[0].innerHTML = String(video.comments);
                   $('#comment_input')[0].innerHTML = video.commentinput;
               }
           });
       }
    }
...
// Videojs: Playlist js Settings
//note in the data-src's above that there are no file extensions, e.g., .m4v
videojs("#video-playlist").ready(function(event){
    var myPlayer=this;
