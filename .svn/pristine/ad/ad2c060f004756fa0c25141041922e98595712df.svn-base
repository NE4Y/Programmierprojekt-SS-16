...
// Abonnierte Playlisten anzeigen

$abbonierteplaylists = Playlist::getAbbonniertePlaylists(User::getUser()->getUserId());

foreach ($abbonierteplaylists as $a_playlist) {
    if ($a_playlist->videos->total > 0) {
        $video = json_decode(Playlist::getVideosByPlaylist($a_playlist->id));
        $a_playlist->{'videolist'} = $video;
    }
}
Controller::$smarty->assign('abbonplay', $abbonierteplaylists);
