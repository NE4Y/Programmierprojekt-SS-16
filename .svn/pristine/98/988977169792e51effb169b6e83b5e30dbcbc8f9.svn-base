...
<section id="recent_videos">
    ...
    <div class="videos">
        {foreach from=$newestvids item=video}
               ...
               <img src="img/play-icon_red.png" />
                ...
                <div class="image_text">
                    ...
                    <label><i class="fa fa-upload" aria-hidden="true"></i>
                        <time id="date_{$video->id}">{$video->uploaddate}</time>
                    </label>
                </div>
                <script>
                    datelabel("date_{$video->id}");
                </script>
            </article>
        {/foreach}
    </div>
       ...
       <span class="play" aria-hidden="true">
        ...
        <div id="video_steps">
            {for $var=1 to {$anzahlVids / $step}}
                <a id="offset_{($var-1) * $step}" href="./videolist?offset={($var-1) * $step}#all_videos">{$var}</a>
            {/for}
        </div>
        <style>
            #offset_{$offset} {
